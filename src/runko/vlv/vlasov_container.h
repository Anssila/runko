...
struct VlasovContainerArgs{
    ...
    std::array<vlv::VlasovGrid::value_type,3> u_init; // The values used to initialize the velocity grid (passed into init_func for every VlasovGrid)
    std::function<void(VGrid&, std::array<vlv::VlasovGrid::value_type,3>)> init_func; // The function used to initialize the velocity grid
};

// Container for holding the Vlasov fluid of a single particle species
// Contains a (spatial) grid of VlasovGrid objects and some common properties of the particles/fluid
template<VelGridType VGrid>
...
private:
    ...
    SpatialGrid grid_; // The actual grid of VlasovGrids
